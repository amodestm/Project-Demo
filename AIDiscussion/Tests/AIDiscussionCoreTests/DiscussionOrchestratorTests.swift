...
final class DiscussionOrchestratorTests: XCTestCase {
    ...
    func testLatestRunPrioritizesRunsWithReceivedUtterances() throws {
        let services = try DiscussionServices.inMemory()
        let group = makeGroup()
        try services.discussionRepo.save(group)

        // 运行 A：稍早启动，但已产生 2 条有效发言
        let runA = DiscussionRun(
            id: "run-A",
            groupID: group.id,
            state: .cancelled,
            startedAt: Date().addingTimeInterval(-100)
        )
        try services.discussionRepo.save(runA)
        let utt1 = DiscussionUtterance(
            runID: runA.id,
            roundIndex: 0,
        ...
        try services.discussionRepo.insert(utt1)
        try services.discussionRepo.insert(utt2)

        // 运行 B：稍晚启动，但在发言前就被取消/意外终止（0 条发言）
        let runB = DiscussionRun(
            id: "run-B",
            groupID: group.id,
            state: .cancelled,
            startedAt: Date().addingTimeInterval(-10)
        )
        try services.discussionRepo.save(runB)

        // 必须优先挑出带真实发言内容的 runA，避免开屏呈现空白页
        let latest = try services.discussionRepo.latestRun(groupID: group.id)
        XCTAssertEqual(latest?.id, runA.id, "开屏载入历史时应优先选择有有效发言内容的运行")

        // 运行 C：当前处于运行中
        let runC = DiscussionRun(
            id: "run-C",
            groupID: group.id,
            state: .running,
            startedAt: Date()
        )
        try services.discussionRepo.save(runC)
