...
一套围绕**实时行情与自动化交易**的个人项目集合，均为 macOS 桌面 / 命令行工具，Python 实现。包含三个互相独立、可单独运行的子项目，合在一起构成从「监控 → 策略 → 执行 → 风控 → 可视化」的完整链路。
...
> `LiquidationMonitor` 与 `QuickTrade` 是从 `LiqHarvest` 中抽取出来的两个子系统，各自配有独立 README 便于单独阅读；`LiqHarvest/` 是自包含的全量代码，可独立运行。三者共用同一套 Binance 行情 / 交易接口约定（密钥走环境变量，不硬编码）。
...
`LiquidationMonitor` 与 `QuickTrade` 各自的启动方式见其目录内的 README。

---

## ⚠️ 安全说明（重要）

**本套件不含任何真实的 API Key / Secret。**
...
本套件仅供学习与研究，**不构成任何投资建议**。加密货币交易具有高杠杆风险，使用本工具产生的任何盈亏由使用者自行承担。请在测试网或小额资金上充分验证后再实盘。
